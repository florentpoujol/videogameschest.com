...
<div id="edittool" class="profile-form update-profile-form">
    <h1>{{ lang('vgc.tool.edit_title') }} <small>{{ $profile->name }}</small></h1>

    <hr>

    <ul class="nav nav-tabs" id="main-tabs">
        <li><a href="#profile-pane" data-toggle="tab">{{ lang('vgc.common.profile') }}</a></li>
        <li><a href="#games-pane" data-toggle="tab">{{ lang('vgc.common.games') }}</a></li>
    </ul>
        ...
        <div class="tab-pane" id="profile-pane">
            ...
            {{ Former::open_vertical(route('post_profile_update', 'tool'))->rules($rules) }} 
                ...
                <div class="row">
                    ...
                    <div class="span8">
                        {{ Former::textarea('pitch', lang('vgc.tool.pitch'))->help(lang('vgc.common.markdown_help'))->class('span8') }}
                    </div>
                </div> <!-- /.row -->
                ...
                {{ Former::primary_submit(lang('vgc.common.update_profile')) }}

            {{ Former::close() }}

        </div> <!-- /.tab-pane #profile-pane -->

        <div class="tab-pane" id="games-pane">

            <p>
                {{ lang('vgc.tool.add_game_relationship_help') }} <br>
                <br>
            </p>
            ...
            {{ Former::open_vertical(route('post_profile_many_relationship_update'))->rules($rules) }}    
                {{ Form::token() }}
                
                {{ Former::hidden('updated_profile_type', $profile->type) }}
                {{ Former::hidden('updated_profile_id', $profile->id) }}
                
                <?php
                $profile_list = $profile->games;
                ?>
                @include('forms/many_relationship_profile_list')
            {{ Former::close() }}
            
        </div> <!-- /.tab-pane #games-pane -->
    </div> <!-- /.tab-content -->
</div> <!-- /#addtool --> 
